...
def next_step(verifying, design, pattern, solution):
    ...
    for poss_sol in solution:
        ...
        if not solution_found and valid:
            #print("check2")
            next_patt = []
            next_three_chars = verifying[poss_sol[1]+1:poss_sol[1]+4]
            
            for i in pattern:
                cut = min(len(i), 3)
                if i[:cut] == next_three_chars[:cut]:
                    next_patt.append(i)
            for i in next_patt:
...
def part1(pattern, design):
    ...
    for check_design in design:
        ...
        solution_found = False
        while True:
            if(solution[0] == 1) or len(solution) == 1 or possible:
                break
            solution, possible = next_step(check_design, design, pattern, solution)
        #print(solution)
        if(possible):
            number_possible += 1
